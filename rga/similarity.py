from .utils import np, combine2Dicts, math, defaultdict, Chem, AllChem, multiplyCoef, Pairs, Torsions, Pool, partial, adjustNoProcessors, divide_work
...
def checkSimilarity(rxnList, config, fout):
    ...
    chunkSz = len(rxnList) // no_processors
    ...
    for sim in simList:
        ...
        if not config.analysisOn:
            fout.write("%*d\t\t%*s\t\t%.5f\n" % (maxCnt, cnt + 1, maxWidth, line, rxnSim))
            cnt += 1
