...
def get_interatomic_distances(conf):
    ...
    # Convert from Angstrom to Bohr (divide by Bohr radius)
    bohr = 0.52917721092
    coords = [(c.x / bohr, c.y / bohr, c.z / bohr) for c in coords]
    d = np.zeros((n_atoms, n_atoms), dtype=float)
    for i in range(n_atoms):
        for j in range(n_atoms):
            if i < j:
                d[i, j] = np.sqrt((coords[i][0] - coords[j][0])**2 + (coords[i][1] - coords[j][1])**2 + (coords[i][2] - coords[j][2])**2)
                d[j, i] = d[i, j]
            else:
                continue
    return d