from utility_module import np, combine2Dicts, math, defaultdict, Chem, AllChem, multiplyCoef, Pairs, Torsions
from coulomb_matrix import coulomb_matrix_eig
...
def checkSimilarity(rxnCnt, rxnFile, simFile, config):
    ...
    with open(rxnFile) as fin, open(simFile, "w") as fout:
        ...
        for line in fin:
                ...
                if featureMethod == "hrcf":
                    ...
                    if featureMethod == "fcfp" or featureMethod == "ecfp":
                        rDicts = [AllChem.GetMorganFingerprint(Chem.MolFromSmiles(r), radius, useFeatures=useFeatures,
                                                         useChirality=useChirality).GetNonzeroElements() for r in reactants]

                        pDicts = [AllChem.GetMorganFingerprint(Chem.MolFromSmiles(p), radius, useFeatures=useFeatures,
                                                         useChirality=useChirality).GetNonzeroElements() for p in products]
                    elif featureMethod == "ap":
                        rDicts = [Pairs.GetAtomPairFingerprint(Chem.MolFromSmiles(r),includeChirality=useChirality).GetNonzeroElements()
                                  for r in reactants]

                        pDicts = [Pairs.GetAtomPairFingerprint(Chem.MolFromSmiles(p), includeChirality=useChirality).GetNonzeroElements()
                                  for p in products]
                    elif featureMethod == "tt":
                        rDicts = [Torsions.GetTopologicalTorsionFingerprint(Chem.MolFromSmiles(r), includeChirality=useChirality).GetNonzeroElements()
                                  for r in reactants]

                        pDicts = [Torsions.GetTopologicalTorsionFingerprint(Chem.MolFromSmiles(p), includeChirality=useChirality).GetNonzeroElements()
                                  for p in products]
            ...
            # reactantSim = cosineSimilarity(rDicts[0], rDicts[1])
            # productSim = cosineSimilarity(pDicts[0], pDicts[1])
            # reactions[line] = [rxnSim, reactantSim, productSim]
