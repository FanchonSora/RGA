from utility_module import defaultdict, Chem, NumValenceElectrons
...
def findBonds(m, coef, config):
    ...
    if ePair or lElectron or ePairType:
        ...
        # Continue the computation of EP_shared and EP_unshared in later parts (bonds and after_bonds)
        if ePairType:
        ...
        for atom in m.GetAtoms():
            ...
            if radical:
                ...
                if radicalElectron > 0:
                    ...
                    rad_bond += "]"
                    bonds[rad_bond] += coef
    # Normal bond: C-O, C=O, C#C
    # Hybridization: C(spx)-C(spy)
    # CDgree: C(3)-C(2)
        ...
        for bond in m.GetBonds():
            ...
            if ePairType:
                if bondType == "=" or bondType == "+":
                    ...
                    bonds['EP_shared'] += 2
            # Normal bond: containing the symbols
            if normalBond:
                ...
                bonds[conn] += coef
            # Hybridization and cDegree add upon the atom symbols
            if bondHybrid or bondHydro or bondDegree or bondRadical:
                sStr = startAtom.GetSymbol()
                eStr = endAtom.GetSymbol()
                # At least one of the connecting atom is a radical
                if bondRadical:
